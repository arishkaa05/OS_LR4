...
int main()
{
	...
	logName = "D:\\OS_LR4\\path1\\logs\\writer_log_" + to_string(processId) + ".txt";
	...
	for (int i = 0; i < PAGES; i++)
	{
		int page = WaitForMultipleObjects(			// ждет, когда массив объектов перейдет в сигнальное состояние
			...
			writerSemaphore,						// массив объектов
			FALSE,									// ждем любого объекта
			INFINITE);

		log << processId << "\t" << page << "\t" << "WRITE_START" << "\t" << GetTickCount64() - start << "\t" << GetTickCount64() << "\t" << start << "\n";
		char* buf = (char*)((char*)viewOfFile + page * pageSize);
		buf[0] = 'a' + i;
		buf[1] = '\0';
		Sleep(500 + rand() % 1000);
		log << processId << "\t" << page << "\t" << "WRITE_END" << "\t" << GetTickCount64() - start << "\t" << GetTickCount64() << "\t" << start << "\n";

		ReleaseSemaphore(							//увеличивает значение счетчика семафора
			readerSemaphore[page],					//идентификатор семафора 
			1,										//значение инкремента 
			NULL);									//адрес переменной для записи 

		log << processId << "\t" << page << "\t" << "RSEM_RELEASED" << "\t" << GetTickCount64() - start << "\t" << GetTickCount64() << "\t" << start << "\n";
	}
